...
public final class RdfConverter {
     ...
     *
     * @param app The ids data app.
     * @return The ids app as rdf string.
     * @throws RdfBuilderException If the response could not be extracted.
     */
    public static String toRdf(final DataApp app) throws RdfBuilderException {
        try {
            var rdf = app.toRdf();
            if (rdf == null || rdf.isEmpty()) {
                rdf = SERIALIZER.serialize(app);
            }
            return rdf;
        } catch (Exception exception) {
            throw new RdfBuilderException(ErrorMessage.RDF_FAILED);
    ...
    /**
     * Get rdf string from instance of type {@link Endpoint}.
     *
     * @param endpoint The ids endpoint.
     * @return The ids endpoint as rdf string.
     * @throws RdfBuilderException If the response could not be extracted.
     */
    public static String toRdf(final Endpoint endpoint) throws RdfBuilderException {
        try {
            var rdf = endpoint.toRdf();
            if (rdf == null || rdf.isEmpty()) {
                rdf = SERIALIZER.serialize(endpoint);
            }
            return rdf;
        } catch (Exception exception) {
            throw new RdfBuilderException(ErrorMessage.RDF_FAILED);
